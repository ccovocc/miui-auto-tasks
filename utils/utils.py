...
def get_token_by_captcha(url: str,use_2captcha: bool) -> Union[str, bool]:
    """通过人机验证码获取TOKEN"""
    try:
        ...
        for attempt in Retrying(stop=stop_after_attempt(6)):
            with attempt:
                ...
                if api_data.success:
                    ...
                    if token := get_token_by_captcha(url,use_2captcha):
                        return token
                    else:
